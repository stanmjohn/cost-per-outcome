...
// name it, and any low, likely, or high written in the program file wins
// over the shipped value.

import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";

const DEFAULT_PATH = fileURLToPath(new URL("../benchmarks/workforce.json", import.meta.url));
...
export function applyBenchmarks(program, benchmarks) {
...
export function benchmarksPage(benchmarks) {
  const lines = [];
  lines.push(`# Workforce benchmarks`);
  lines.push("");
  lines.push(`Built from \`benchmarks/workforce.json\` by \`node bin/cost-per-outcome.mjs benchmarks\`. Every number is a default, never a fact. A program file can override any of them.`);
  for (const e of benchmarks) {
    lines.push("");
    lines.push(`## ${e.label}`);
    lines.push("");
    lines.push(`Name in a program file: \`${e.id}\`. Unit: ${e.unit}.`);
    lines.push("");
